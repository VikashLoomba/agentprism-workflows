...
const smoke = await agent(
  `Live end-to-end smoke test of the integrated orchestrator at ${REPO} (read ${P3} for the integration commit). Ensure the repo is on ${BASE_BRANCH} (the integrated build branch). Build if needed, then start the mcp-server and call its "workflow" tool with a TRIVIAL one-agent script that uses a small schema, e.g. meta + \`const r = await agent("Return the repo name and file count as JSON", {schema: SMALL})\`. Run it once routed to a REAL claude-agent-acp (Claude path) and once routed to the PATCHED codex-acp (Codex path). Assert each returns a schema-VALIDATED structured object (not text) and that notifications/progress fired. Report claudePass, codexPass, details, and any errors. If a backend cannot authenticate in this environment, report that explicitly rather than marking pass.`,
