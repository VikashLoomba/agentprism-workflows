...
const BASE_BRANCH = 'build/acp-mcp' // the integration base that holds the TOTALITY of the build; all merges + the integration commit land here
const MAX_ROUNDS = 4
...
for (let round = 1; round <= MAX_ROUNDS; round++) {
  status = await agent(
    round === 1
      ? `Integrate the three Phase-2 modules in the MAIN repo at ${REPO}. Read ${P2} (= {baseSha, modules:[{module,branch,commitSha,worktreePath}]}). FIRST ensure the main repo is on the integration base branch ${BASE_BRANCH} (which holds the TOTALITY of the build): run "git -C ${REPO} checkout ${BASE_BRANCH}" and verify "git -C ${REPO} rev-parse --abbrev-ref HEAD" equals ${BASE_BRANCH}; if it cannot, STOP. ALL merges and the integration commit MUST land on ${BASE_BRANCH}. MERGE BY COMMIT SHA (more robust than branch name): for each module run \`git -C ${REPO} merge --no-ff <commitSha> -m "integrate <module>"\`, all three sequentially. They touch DISJOINT packages/<module>/ dirs and none committed root files or the lockfile, so merges MUST be clean — if git reports a conflict it almost certainly means a module violated the "only its package dir" rule, so STOP and report it rather than force-resolving. If a commitSha is unreachable (\`git cat-file -t\` fails), fall back to applying that module's diff from its worktreePath under ${WT}. AFTER all three merge: run a SINGLE \`pnpm install\` at the repo root to regenerate pnpm-lock.yaml from the three merged packages/*/package.json (the lockfile was intentionally NOT committed per-branch to avoid a 3-way lockfile conflict). Then build the whole workspace (\`pnpm -r build\` or \`tsc -b\`), ensure mcp-server's composition root wires an acp-agents AgentRunner into the engine (DI). RECONCILE the known Phase-2 seam mismatch (read each module's gapsLeft + handoffNotes in ${P2}): mcp-server was built against the scaffold STUB engine, but the REAL workflow-engine exposes WorkflowManager.runSync(script, args?, exec) which RESOLVES to a terminal host-facing WorkflowRunResult (status/reason/resetHint; does NOT throw on pause/fail/abort) plus RunPersistence for resume. Route the mcp-server workflow-tool handler through WorkflowManager.runSync and DROP mcp-server's shell-side status-composition try/catch and its shell-owned resume emulation, letting the engine own status stamping + resumeFromRunId->resumeJournal. That is the CORRECT reconciliation, NOT a compile-patch. NAMING CONSOLIDATION: the engine port left a mix of .pi/* and .agentprism/* runtime paths — consolidate ALL remaining .pi/* to .agentprism/* across workflow-engine src (run persistence .pi/workflows->.agentprism/workflows, worktrees .pi/worktrees->.agentprism/worktrees, any leftover .pi/agents->.agentprism/agents, the run branch prefix pi/wf/<slug>->agentprism/wf/<slug>, and ~/.pi/workflows/model-tiers.json->~/.agentprism/workflows/model-tiers.json). These are directory/branch names ONLY — they do NOT enter hashAgentCall/resume identity, so it is safe; verify NO '.pi/' or 'pi/wf' literal remains in workflow-engine src afterward. And run tests (\`pnpm -r test\`). Report tscOk, testsOk, mergedBranches, and a PRECISE list of remaining failures (where = file:line/package, msg). DEFINITION OF DONE (strict): Fully RESOLVE every build/test/type/seam failure — never stub, comment out, 'as any', @ts-ignore, .skip a test, or insert a TODO/'follow-up' to force a green build. If a failure exposes a genuine contradiction with the frozen contract, STOP and surface it rather than papering over it.`
...
const rec = await agent(
  `Commit the integrated workspace at ${REPO} (INCLUDING the regenerated pnpm-lock.yaml) on the ${BASE_BRANCH} branch (the integration base that holds the totality of the build) with message "integrate: phase-2 modules onto frozen scaffold". Then clean up the now-unneeded Phase-2 worktrees (the phase2/* branches remain in history): for m in workflow-engine acp-agents mcp-server, run \`git -C ${REPO} worktree remove --force ${WT}/$m\` (ignore if already gone). Finally WRITE ${ORCH}/phase3-integration.json = {integrationCommitSha, tscOk, testsOk, remainingFailures, notesForPhase4, handoffPath} reflecting the final integration status:\n${JSON.stringify(status)}`,
