...
and the package READMEs for what already exists.

## Status legend

| Status | Meaning |
| --- | --- |
| **implementing** | Design settled; staged implementation is landing |
| **implemented-by-this-contract** | The linked design record is implemented; retained here for design provenance until release notes replace the row |
...
| --- | --- | --- |
| [Run events: typed contract & durable event log](docs/roadmap/run-events.md) | implementing | Typed `RunEvent` payloads, direct ACP call correlation, and a generation-pinned append-only tail are landing in staged PRs |
