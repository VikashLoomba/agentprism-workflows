...
describe("parseAgentDefinition", () => {
  ...
  it("drops empty/blank tool entries (minimal frontmatter parser keeps bare scalars as strings)", () => {
    // The de-Pi'd, dependency-free frontmatter parser does NO YAML type inference:
    // every inline-array scalar is a string (a bare `3` becomes "3", not a number,
    // and a quoted '' / '  ' becomes an empty/blank string). toStringArray then drops
    // only empty / whitespace-only entries; non-empty tokens are kept verbatim.
    const md = "---\nname: x\ntools: [read, 3, '', '  ', write]\n---\nbody";
    const def = parseAgentDefinition(md, "project", "x.md");
    assert.deepEqual(def?.tools, ["read", "3", "write"]);
  });
...
// ── loadAgentRegistry (dir injection — now .agentprism/agents by default) ───
...
describe("agentDefinitionKey", () => {
  ...
  it("serializes the EXACT, UNCHANGED key shape (so a Pi-computed resume hash still matches)", () => {
    // The serialization is deliberately frozen across the de-Pi lift: a resume hash
    // computed under Pi must still match. Pin the literal key (field order + null
    // sentinels for absent tools/disallowedTools/model/isolation; source/name/description
    // are NOT part of identity).
    const def: AgentDefinition = { name: "x", prompt: "p", tools: ["read"], source: "project" };
    assert.equal(
      agentDefinitionKey(def),
      '{"tools":["read"],"disallowedTools":null,"model":null,"isolation":null,"prompt":"p"}',
    );
    const full: AgentDefinition = {
      name: "y",
      description: "ignored for identity",
      prompt: "body",
      tools: ["read", "grep"],
    ...
    assert.equal(
      agentDefinitionKey(full),
      '{"tools":["read","grep"],"disallowedTools":["bash"],"model":"vendor/m","isolation":"worktree","prompt":"body"}',
    );
  });
...
// The injected runner is the frozen AgentRunner seam (run(prompt, options)); the
// engine calls it once per agent() and forwards model/tier/toolNames/instructions/cwd.
...
return {}`;
  ...
  it("agentType isolation: worktree runs the agent in an isolated cwd", async () => {
    const repo = mkdtempSync(join(tmpdir(), "ap-agent-isolation-"));
...
return {}`;
      ...
      assert.ok(seen[0].cwd?.includes(join(".agentprism", "worktrees")), "isolated cwd is under .agentprism/worktrees");
